...
public class DayGatherRptAction implements Serializable {
    ...
    private String totalBudgetCityIncome = "0.00";            // 预算内合计
    private String totalFundCityIncome = "0.00";            // 预算外合计 [对账成功的不明款转票据(注意：不明款有两种处理)]
    private String totalUnknownIncome = "0.00";            //  当日不明款转收入合计

    private String vchBudgetCityIncome = "0.00";               // 预算内本级正常收入
    private String pendingBudgetCityIncome = "0.00";           // 预算内本级待补票收入
    private String unknownBudgetCityIncome = "0.00";           // 预算内本级不明款收入

    private String vchFundCityIncome = "0.00";               // 预算外本级正常收入
    private String pendingFundCityIncome = "0.00";           // 预算外本级待补票收入
    private String unknownFundCityIncome = "0.00";           // 预算外本级不明款收入
        ...
        try {
            ...
            for (DayGatherData data : dayGatherList) {
                ...
                if ("01".equals(data.getZjxzbm()) || "1".equals(data.getZjxzbm())) {                                     // 01-预算内
                    ...
                    totalBudgetBDIncome = totalBudgetBDIncome.add(bjAmt);          // 本级收入合计
                    // 收入属性
                    ...
                    totalFundBDIncome = totalFundBDIncome.add(bjAmt);               // 本级收入合计
                    // 收入属性
                    if ("1".equals(data.getPendingflag())) {   // 补票
                        record.setIncomeProperty("待补票转收入");
                        totalPendingBDIncome = totalPendingBDIncome.add(record.getZje());
